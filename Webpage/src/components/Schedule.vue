...
const { people } = storeToRefs(globals);
...
const hours = ref(["12AM", "1AM", "2AM", "3AM", "4AM", "5AM", "6AM", "7AM", "8AM", "9AM", "10AM", "11AM", "12PM", "1PM", "2PM", "3PM", "4PM", "5PM", "6PM", "7PM", "8PM", "9PM", "10PM", "11PM"]);
...
const schedulePeople = ref(people.value);
...
function convertToScheduleObject() {
  schedulePeople.value.forEach(s => {
    s.schedule = [];
  });
...
<template>
  ...
  <div class="w-full h-full bg-slate-200" v-else>
    <div class=" w-full h-24 flex items-center justify-center text-4xl text-slate-600 border-b-2 border-gray-300 relative">
      ...
      <div class="absolute top-5 right-5 py-2 px-5 rounded bg-indigo-500 text-white text-lg">Edit</div>
    </div>
    <div class="w-full flex">
      <div class="flex flex-col justify-start w-60">
        ...
        <div class="w-full h-28 flex justify-start items-center p-3 bg-white border-r-4 border-b-4 gap-3 border-gray-300" v-for="(p, i) in schedulePeople" :key="i">
      ...
      <div class="flex flex-col justify-start w-[calc(100%-15rem)]">
        <div class="w-full overflow-x-auto overflow-y-hidden">
          <div class="w-fit h-16 flex justify-center items-center text-gray-500 font-bold border-gray-300 border-b-4 relative">
            <div class="border-x border-gray-300 w-40 h-full flex items-center justify-center" v-for="(h, i) in hours" :key="i">{{ h }}</div>
          ...
          <div class="w-fit h-28 flex justify-start items-center bg-white border-b-4 border-gray-300 relative" v-for="(p, i) in schedulePeople" :key="i">
            <div class="w-40 h-28" v-for="(h, i) in hours" :key="i"></div>
            <div class="absolute w-full h-full">
              <div v-for="(s, index) in p.schedule" :key="index" class="h-24 absolute top-[6px] rounded text-white flex text-center justify-center items-center break-words border-x-2 border-white border-opacity-20" :style="'left: ' + (s.start * 10) + 'rem; width: ' + ((s.end - s.start) * 10) + 'rem; background-color: ' + p.color + ';'">{{ s.name }}</div>
            </div>
          </div>
        </div>
      </div>
