...
function checkPassword() {
  if (passcodeInput.value == adminPasscode.value) {
    ...
    setTimeout(() => { passcodeWrong.value = false; }, 3000);
  }
}
</script>
...
<template>
  <div class="w-full h-full flex flex-col justify-center items-center bg-slate-200">
    ...
    <div class="w-64">
      <div class="w-full h-16 bg-slate-300 rounded flex justify-center items-center mb-3 text-2xl">{{ passcodeInput }}</div>
      <div class="flex flex-wrap gap-2">
        <div class="w-20 h-20 flex justify-center items-center text-white rounded text-2xl font-bold" :class="n == ' ' ? 'bg-slate-200' : 'bg-slate-500'" v-for="n in numbers" :key="n" @click="numPressed(n)">{{ n }}</div>
      </div>
